#!/bin/tcsh

set BxCutValue = 2

set PreNdsaCutValue = 6
set PrePtCutValue = 10.
set PreChaCutValue = 1
#set PreChaCutValue = 0
#set PreChaCutValue = 2
set PreEtaCutValue = 1.0
#set PreRpcCutValue = 0
set PreRpcCutValue = 1
#set PreRpcCutValue = 2
#set PreRpcCutValue = 3
#set PreDtHitsCutValue = 0
#set PreDtHitsCutValue = 8
set PreDtHitsCutValue = 7
#set PreDtHitsCutValue = 0
set PreInvBetaErrCutValue = 10.
set PreCscHitsCutValue = 0

#set PtCutValue = 10.
#set ChaCutValue = 1
#set EtaCutValue = 1.0
#set RpcCutValue = 1



#set PtCutValue = 20.
set PtCutValue = 30.
#set PtCutValue = 25.
#set ChaCutValue = 0
###set ChaCutValue = 1
set ChaCutValue = 2
#set ChaCutValue = 3
set EtaCutValue = 1.0
#set RpcCutValue = 0
#set RpcCutValue = 1
set RpcCutValue = 2
#set RpcCutValue = 3
#set RpcCutValue = 4
#set RpcCutValue = 5
#set DisStCutValue = 1
set DisStCutValue = 2
set DtHitsCutValue = 7
#set DtHitsCutValue = 0
set CscHitsCutValue = 0
#set DtInvBetaCutValue = 0.2
#set DtInvBetaCutValue = 0.0
set DtInvBetaCutValue = -0.5
#set TimeInOutCutValue = 0.
#set TimeInOutCutValue = -20.
#set TimeInOutCutValue = -15.
set TimeInOutCutValue = -10.
set TimeInOutErrCutValue = 2.
###set TimeInOutErrCutValue = 5.
#set DeltaTimeInOutCutValue = -18.
set DeltaTimeInOutCutValue = -22.
#set DeltaTimeInOutCutValue = -25.
#set DeltaTimeInOutCutValue = -30.
#set DeltaTimeInOutCutValue = -32.
#set DeltaTimeInOutCutValue = -35.
set DeltaDtInvBetaCutValue = -1
#set TimeInOutErrCutValue = 4.
set OppEtaCutValue = 0.1
#set OppEtaCutValue = 0.6
set OppPhiCutValue = 0.1
#set OppPhiCutValue = 0.5
#set OppPhiCutValue = 1.0
#set OppPhiCutValue = 2.0
#set OppPhiCutValue = 3.0
set DxyCutValue = 50

#set abcdInvBetaCutValue = -1;
#set abcdInvBetaCutValue = -0.5;
#set abcdInvBetaCutValue = -0.2;
#set abcdInvBetaCutValue = 0.0;
#set abcdInvBetaCutValue = 0.2;
#set abcdInvBetaCutValue = 0.3;
set abcdInvBetaCutValue = 0.5;
#set abcdInvBetaCutValue = 0.6;
#set abcdInvBetaCutValue = 0.7;
#set abcdInvBetaCutValue = 0.8;
#set abcdInvBetaCutValue = 0.9;
#set abcdInvBetaCutValue = 1.0;
#set abcdInvBetaCutValue = 1.3;
#set abcdInvBetaCutValue = 1.6;

#set abcdPtCutValue = 35.0;
#set abcdPtCutValue = 40.0;
#set abcdPtCutValue = 41.0;
#set abcdPtCutValue = 42.0;
#set abcdPtCutValue = 43.0;
#set abcdPtCutValue = 44.0;
#set abcdPtCutValue = 45.0;
#set abcdPtCutValue = 50.0;
set abcdPtCutValue = 51.0;
#set abcdPtCutValue = 60.0;
#set abcdPtCutValue = 70.0;
#set abcdPtCutValue = 80.0;
#set abcdPtCutValue = 90.0;
#set abcdPtCutValue = 100.0;
#set abcdPtCutValue = 110.0;
#set abcdPtCutValue = 120.0;
#set abcdPtCutValue = 140.0;
#set abcdPtCutValue = 150.0;
#set abcdPtCutValue = 170.0;
#set abcdPtCutValue = 200.0;
#set abcdPtCutValue = 250.0;
#set abcdPtCutValue = 300.0;
#set abcdPtCutValue = 350.0;

set abInvBetaCutValue = -0.8;
set cInvBetaCutValue = 0.8;
set acPtCutValue = 60.0;
set bPtCutValue = 200.0;

set cut_values = "$BxCutValue $PreNdsaCutValue $PrePtCutValue $PreChaCutValue $PreEtaCutValue $PreRpcCutValue $PreDtHitsCutValue $PreInvBetaErrCutValue $PreCscHitsCutValue $PtCutValue $ChaCutValue $EtaCutValue $RpcCutValue $DisStCutValue $DtHitsCutValue $CscHitsCutValue $DtInvBetaCutValue $TimeInOutCutValue $TimeInOutErrCutValue $DeltaTimeInOutCutValue $DeltaDtInvBetaCutValue $OppEtaCutValue $OppPhiCutValue $DxyCutValue $abcdInvBetaCutValue $abcdPtCutValue $abInvBetaCutValue $cInvBetaCutValue $acPtCutValue $bPtCutValue"
