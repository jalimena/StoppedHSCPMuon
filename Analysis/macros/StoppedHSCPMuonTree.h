...
class StoppedHSCPMuonTree {
public :
   TTree          *fChain;   //!pointer to the analyzed TTree or TChain
   Int_t           fCurrent; //!current Tree number in a TChain

   // Declaration of leaf types
 //StoppedHSCPMuonEvent *events;
   UInt_t          rHadPdgId;
   Double_t        rHadVtxX;
   Double_t        rHadVtxY;
   Double_t        rHadVtxZ;
   ...
   StoppedHSCPMuonTree(TTree *tree=0);
   virtual ~StoppedHSCPMuonTree();
...
#ifdef StoppedHSCPMuonTree_cxx
StoppedHSCPMuonTree::StoppedHSCPMuonTree(TTree *tree)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     //TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/storage/phjjb/stoppedHSCP/stoppedHSCP_tree_Run2010A_Apr21_423p5_v1_1117_1309/stoppedHSCPTree_1_1_6k8.root");
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/afs/cern.ch/user/j/jalimena/public/Ntuples/stoppedHSCPMuonTree_onlyNoBPTX_190465_ls68_151.root");
      if (!f) {
	//f = new TFile("/storage/phjjb/stoppedHSCP/stoppedHSCP_tree_Run2010A_Apr21_423p5_v1_1117_1309/stoppedHSCPTree_1_1_6k8.root");
	f = new TFile("/afs/cern.ch/user/j/jalimena/public/Ntuples/stoppedHSCPMuonTree_onlyNoBPTX_190465_ls68_151.root");
      }
      tree = (TTree*)gDirectory->Get("StoppedHSCPMuonTree");

   }
   Init(tree);
}

StoppedHSCPMuonTree::~StoppedHSCPMuonTree()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t StoppedHSCPMuonTree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t StoppedHSCPMuonTree::LoadTree(Long64_t entry)
{
...
void StoppedHSCPMuonTree::Init(TTree *tree)
{
...
Bool_t StoppedHSCPMuonTree::Notify()
{
...
void StoppedHSCPMuonTree::Show(Long64_t entry)
{
...
Int_t StoppedHSCPMuonTree::Cut(Long64_t entry)
{
...
bool StoppedHSCPMuonTree::triggerCut() const {      // require event passed main trigger
...
bool StoppedHSCPMuonTree::bptxVeto() const {        // cut on time wrt BPTX signal
  return true;
}

bool StoppedHSCPMuonTree::bxVeto() const {          // cut on BX wrt expected collisions
  return abs(bxWrtBunch) > 2;
}

bool StoppedHSCPMuonTree::vertexVeto() const {      // no vertex
  return (nVtx == 0);
}

bool StoppedHSCPMuonTree::haloVeto() const {        // no halo ID
  return !(beamHalo_CSCLoose) ;//&& geometryHaloCut();
}

bool StoppedHSCPMuonTree::hfVeto() const {
  return hfPlusTotalE < 50. && hfMinusTotalE < 50.;
}

bool StoppedHSCPMuonTree::trackVeto() const {
  return (track_N == 0);
}

bool StoppedHSCPMuonTree::cosmicVeto() const {      // no cosmic muon
  return mu_N==0;
}

bool StoppedHSCPMuonTree::hcalNoiseVeto() const {   // std HCAL noise veto
  //std::cout <<"TRIGDEC:"<<run<<":"<<lb<<":"<<id<<":"<<bx<<":"<<noiseFilterResult<<std::endl;
  return noiseFilterResult;
}

bool StoppedHSCPMuonTree::looseJetCut() const {     // low Et threshold
  return jet_N>0 && jetE[0]>30. && fabs(jetEta[0])<1.3;
}

bool StoppedHSCPMuonTree::jetEnergyCut() const {    // require jet above Et threshold
  // raise jet energy cut to 70, since we're using a 50-GeV trigger?
  return jet_N>0 && jetE[0]>70. && fabs(jetEta[0])<1.3;
}

bool StoppedHSCPMuonTree::jetN60Cut() const {       // jet n60
  return jet_N>0 && jetN60[0]<6;
}

bool StoppedHSCPMuonTree::jetN90Cut() const {       // jet n90
  return jet_N>0 && jetN90[0]>3;
}

bool StoppedHSCPMuonTree::towersIPhiCut() const {   // cut on N leading towers at same iphi
  return nTowerSameiPhi<5 && leadingIPhiFraction()<0.95;
}

bool StoppedHSCPMuonTree::iPhiFractionCut() const { // Et fraction at leading iphi
  return leadingIPhiFraction()<0.95;
}

bool StoppedHSCPMuonTree::hpdR1Cut() const {        // timing R1 cut from HPD
  return (topHPD5R1 > 0.15) && (topHPD5R1 <= 1.0);
}

bool StoppedHSCPMuonTree::hpdR2Cut() const {        // timing R2 cut from HPD
  return (topHPD5R2 > 0.1) && (topHPD5R2 < 0.8);
}

bool StoppedHSCPMuonTree::hpdRPeakCut() const {     // timing Rp cut from HPD
  return (topHPD5RPeak > 0.3) && (topHPD5RPeak < 0.7) && (topHPD5PeakSample > 0) && (topHPD5PeakSample < 7);
}

bool StoppedHSCPMuonTree::hpdROuterCut() const {    // timing Ro cut from HPD
  return (topHPD5ROuter < 0.3) && (topHPD5ROuter >= 0.0) && (topHPD5PeakSample > 0) && (topHPD5PeakSample < 7);
}

double StoppedHSCPMuonTree::leadingIPhiFraction() const {
...
#endif // #ifdef StoppedHSCPMuonTree_cxx
