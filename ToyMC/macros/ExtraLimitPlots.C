...
ExtraLimitPlots::ExtraLimitPlots(double tpLumi) :
  ...
  readMchampData(std::string("mcMchamp.txt"));
...
void ExtraLimitPlots::readMchampData(std::string filename) {

  std::cout << "Reading mchamp data from " << filename << std::endl;
  ...
  while (file >> m >>  er >> xst) {
          ...
          //mchampChi0Mass.push_back(mchi);
          //mchampEffStop.push_back(es);
//           stopEffStopNB.push_back(esnb);
//           stopEffStopEM.push_back(esem);
...
void ExtraLimitPlots::calculateCrossSections(unsigned gluinoIndex, 
  ...
  std::cout << "  mmchamp=" << mchampMass[mchampIndex] 
  ...
  for (unsigned i=0; i<mchampMass.size(); ++i) {
    ...
    if (mchampEffReco[i] > 0.) {
      ...
      // observed mchamp
      xsProdMassMchamp.push_back( toyObsLimit[toyIndex] / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      if (tpLifetime.size()>tpIndex)
      	xsProdMassTPMchamp.push_back( tpObsLimit[tpIndex] / (tpLumi_ * mchampEffReco[i]) );
      
      // expected mchamp
      xsProdExpMassMchamp.push_back( toyExpLimit[toyIndex] / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      xsProdExpLo1SigMassMchamp.push_back( (toyExpLimit[toyIndex]-toyExpLimitLo1Sig[toyIndex]) / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      xsProdExpHi1SigMassMchamp.push_back( (toyExpLimitHi1Sig[toyIndex]-toyExpLimit[toyIndex]) / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      xsProdExpLo2SigMassMchamp.push_back( (toyExpLimit[toyIndex]-toyExpLimitLo2Sig[toyIndex]) / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      xsProdExpHi2SigMassMchamp.push_back( (toyExpLimitHi2Sig[toyIndex]-toyExpLimit[toyIndex]) / (toyEffLumi[toyIndex] * mchampEffReco[i]) );
      
      std::cout << " mass=" << mchampMass.at(i) << std::endl;
...
TGraphAsymmErrors* ExtraLimitPlots::getExpLimitMchamp2Sig() {
	return new TGraphAsymmErrors(xsProdExpLimitToyMchamp.size(),
				     &toyLifetime[0],
				     &xsProdExpLimitToyMchamp[0],
				     0,
...
TGraphAsymmErrors* ExtraLimitPlots::getExpMassLimitMchamp2Sig() {
	return new TGraphAsymmErrors(xsProdExpMassMchamp.size(),
								&massMchamp[0],
								&xsProdExpMassMchamp[0],
								0,
...
TGraph* ExtraLimitPlots::getMchampTheory() {
