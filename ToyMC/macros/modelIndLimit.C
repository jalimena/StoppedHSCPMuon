...
void modelIndLimit(double lumi=-1.,double maxInstLumi=-1.) 
{
  ...
  TGraph* g_mchamp  = plots.getLimitMchampBasic();
  ...
  //h->SetTitle("Beamgap Expt;#tau [s]; Model Independent Cross-section [pb]");
  h->SetTitle("Beamgap Expt;#tau [s]; #sigma #times #Beta #times #varepsilon_{stopping} #times #varepsilon_{reco} [pb]");
  ...
  leg->SetTextFont(42);
  leg->SetFillColor(0);
  leg->AddEntry(g_gluino, "Gluino : m_{#tilde{g}}=500 GeV, m_{#tilde{#chi^{0}}}=387 GeV", "l");
  leg->AddEntry(g_stop, "Stop  : m_{#tilde{t}}=300 GeV, m_{#tilde{#chi^{0}}}=109 GeV", "l");
  leg->AddEntry(g_mchamp, "Mchamp  : m_{mchamp}=500 GeV", "l");
  leg->Draw();
  ...
  g_mchamp->SetLineColor(6);
  g_mchamp->SetLineStyle(1);
  g_mchamp->SetLineWidth(2);
  g_mchamp->Draw("l");
